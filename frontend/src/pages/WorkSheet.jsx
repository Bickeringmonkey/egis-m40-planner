...
function WorkSheet() {
  ...
  const loadWorksheet = async () => {
    ...
    if (!date || !workstreamId) {
      setMessage("Please select a date and workstream.");
      return;
    }

    try {
      const res = await api.get(`/work-sheet?date=${date}&workstreamId=${workstreamId}`);
      setJobs(res.data);
    } catch (err) {
      ...
      <div className="worksheet-actions print-hide">
        <div>
          ...
          <p className="page-subtitle">
            Select a date and workstream to create a printable worksheet.
          </p>
        </div>
...
/>
    ...
    <div>
      ...
      <div className="worksheet-brand-subtitle">Egis Road Operations</div>
    </div>
  </div>
  ...
  <div className="worksheet-title-block">
    ...
    <p>
      ...
      <strong>Workstream:</strong> {selectedWorkstream || "Not selected"}
    </p>
  </div>
  ...
  <div className="worksheet-count">
    {jobs.length} job{jobs.length !== 1 ? "s" : ""}
              ...
              {jobs.length > 0 ? (
                jobs.map((job) => (
                  <tr key={job.id}>
                    ...
                    {/*<td>{job.nems_number || ""}</td> */}
                    {/* <td className="col-c'way">{job.carriageway || ""}</td> */}
                    {/* <td className="col-junctions">{job.junctions_between || ""}</td> */}
      ...
      <td colSpan={9}>No jobs found for this date and workstream.</td>
    </tr>
              )}
            </tbody>
          </table>
