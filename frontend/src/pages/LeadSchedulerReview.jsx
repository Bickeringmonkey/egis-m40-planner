...
function LeadSchedulerReview() {
  ...
  const saveJob = async (job) => {
    try {
      ...
      await api.put(`/jobs/${job.id}/lead-scheduler-check`, {
        lead_scheduler_checked: !!job.lead_scheduler_checked,
      });

      setMessage(`Final completion saved for ${job.job_number}.`);
      loadJobs();
    } catch (err) {
      console.error(err);
      setMessage(err.response?.data?.error || "Failed to save final completion.");
    }
  };
  ...
  return (
    <div className="list-page list-page-compact">
      <div className="list-page-header list-page-header-tight">
        <div>
          <h1 className="page-title">Lead Scheduler Final Completion</h1>
          <p className="page-subtitle">
            Final check and completion sign-off for completed night works.
          </p>
        </div>
      </div>
      ...
      {!loading && jobs.length > 0 && (
        <div className="list-table-card">
          <div className="list-table-header">
            <h2>Jobs for Final Completion</h2>
...
export default LeadSchedulerReview;